...
from settings_multiple_S_simu import QRVCM  # it can exclude bar_A_it
from settings_multiple_S_simu import ker 
...
y_it = df.loc[:,'y'].values.reshape(dates, m) / np.std(df.loc[:, 'y'])
S_it ={}
S_it['S1'] = df.loc[:,'S1'].values.reshape(dates, m) / np.std(df.loc[:, 'S1'])
S_it['S2'] = df.loc[:,'S2'].values.reshape(dates, m) / np.std(df.loc[:, 'S2'])
...
for tau in taus:
    model = QRVCM(  y_it, A_it, S_it,time_points, tau, hc, hcb )
    ...
    df_output_y = { 'y':y_it.reshape(-1,1, order='F'), 'fitted_y':model.fitted_vY, 'resid_y':resid_vY}
    ...
    df_output_S1 = { 'S1':S_it['S1'][:,np.arange(1,m)].reshape(-1,1,order='F'),
                    'fitted_S1':model.fitted_mS[:, 0], 'resid_S1':resid_mS[:, 0]}
    ...
    df_output_S2 = { 'S2':S_it['S2'][:,np.arange(1,m)].reshape(-1,1,order='F'),
                    'fitted_S2':model.fitted_mS[:, 1], 'resid_S2':resid_mS[:, 1]}
    ...
    ######################
    file_name='./Results/'+'Testing_'+str(tau)+'.csv'
    file_name_1 ='./Results/'+'y_tau_'+str(tau)+'.csv'
    file_name_2 ='./Results/'+'S_tau_'+str(tau)+'.csv'
    file_name_3 ='./Results/'+'Est_tau_'+str(tau)+'.csv'

...
model0 = LRVCM( y_it, A_it_tmp, S_it, time_points, hc, hcb )
...
beta_1 = coes_0['coe_y_smoothed_0'].drop(['const'], axis=0).values 
...
for t in np.delete(range(m), m-1):
    ...
    for index_response in covariate_index:
        if index_response==covariate_index[0]:
            phi['t='+str(t)] =coes_0['coe_s_smoothed_0'][index_response].drop(['const']).loc[:, 't='+str(t)].values.reshape(1,d) 
        else:
            phi['t='+str(t)] = np.vstack((phi['t='+str(t)], coes_0['coe_s_smoothed_0'][index_response].drop(['const']).loc[:, 't='+str(t)].values.reshape(1,d)   ))
 
QDE_true, QIE_true= model0.QDE_QTE_calculate( time_start=0, time_end=m, gamma_y=gamma_y, Gamma_s=Gamma_s, phi=phi, beta_1=beta_1)

########### true coes ######################

coe_y_true = np.vstack( (coes_0['coe_y_smoothed_0'].loc['const',:].values.reshape(1, m), gamma_y.reshape(1,m), beta_1 ) )
...
def Simulation_run(R):
    ...
    for r in R:
       ...
       model = LRVCM( y_it_select, sample_A_it,  S_it_select, time_points,  hc, hcb )
...
if __name__ == '__main__':
    ...
    #from QRVCM_inference import QRVCM 
...
######### calculate statistics ####
 ...
 ########## results output ######################
    folder_path ='./Linear/ResidMultiple_NN_'+str(NN)+'_TI_'+str(TI)

...
class QRVCM(object):
    ...
    def estimate(self, y_it=None, S_it=None, A_it=None, hc=None):
        ...
        data_t = {'y':self.vY.ravel(), 'A':self.vA.ravel() }
        nS=0
        for index in self.covariate_index:
            data_t[index]= self.mS[:, nS]
            nS=nS+1
            reg_term=reg_term + '+'+index

        data_t=pd.DataFrame(data_t)    
        quan_model_t = smf.quantreg(reg_term, data_t).fit(q=self.tau)
        coe_y_smoothed  = quan_model_t.params
                
        fitted_vY = quan_model_t.fittedvalues
        resid_vY = quan_model_t.resid
        
        self.coe_y_smoothed = coe_y_smoothed 
        self.fitted_vY = fitted_vY.to_numpy().reshape(-1,1) 
        self.resid_vY = resid_vY.to_numpy().reshape(-1,1) 

                               
        return coe_y_smoothed, resid_vY
    ...
    def testing(self, B, time_start, time_end, hcb=None):
        ...
        coe_y_smoothed, resid_vY = self.estimate()
        QDE = coe_y_smoothed['A']

        resid_vY = resid_vY - np.quantile(resid_vY, q=self.tau)
        ...
        for b in range(B):
            ...
            positions = choices( list(range(self.n*self.m)),  k=self.n*self.m )


            resid_vY_b = resid_vY[positions]

            
            fitted_vY_b  = self.fitted_vY + resid_vY_b.to_numpy().reshape(-1,1)
            ...
            coe_y_smoothed_b, resid_vY_b = model_b.estimate()
                                       
            QDE_b=coe_y_smoothed_b['A']
        ...
        pvalues_all = np.array([[ pvalue_QDE_twoSide,  pvalue_normal_QDE_twoSide ], [  pvalue_QDE_RightSide,  pvalue_normal_QDE_RightSide ] ])
        
        pvalues_all = pd.DataFrame(pvalues_all, columns=['QDE_boot', 'QDE_normal'], index=['Two_sides', 'Right_side'])

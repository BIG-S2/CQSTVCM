...
import time
from time import *
path='./quantile_temporal_comparison'
import sys
if path not in sys.path:
    sys.path.append(path)    
from noInterference_compare import QRVCM
...
#########################################################
df = pd.read_csv('simu_demo_datacsv', index_col=['date','time'])
...
#### Output true coes ######################
folder_path_coe ='./True_coes'
    
import os
if not os.path.exists(folder_path_coe):
   os.makedirs(folder_path_coe)
   
col_coes = [ 't='+str(s) for s in range(m)]
    
index_coes = [ 'beta_0', 'gamma_y', 'beta_11', 'beta_12', 'phi_01', 'Gamma_s1', 'phi_11', 'phi_12',
              'phi_01', 'Gamma_s1', 'phi_11', 'phi_12']

coes_true_all = np.vstack((coe_y_true, np.array(coe_s1_true),np.array(coe_s2_true) ))
    
coes_all_true = pd.DataFrame( coes_true_all, columns=col_coes, index=index_coes ) 


file_coe = folder_path_coe  + '/coes_all_true.csv'    # + cityid
    
coes_all_true.to_csv(file_coe)

del coe_s1_true['t=m-1']
del coe_s2_true['t=m-1']
#############################
def Simulation_run(R):
    ...
    for r in R:
       ...
       coe_y_smoothed, resid_vY=model.estimate()
       QDE = coe_y_smoothed['A']

       run_start=time()
       pvalues  = model.testing(B=boot_times, time_start=0, time_end=m)      
       run_end = time()
...
###### calculate MSEs of coefficients ####
       ...
       results_tmp =np.array( [ [QDE, run_time] ] )
       
       results_tmp  = np.hstack( (np.array(pvalues).reshape(1, 4), results_tmp) )
...
######### calculate statistics ####
    ...
    results_all = results #np.hstack( (results, (( results[:, 8] - QDE_true )**2).reshape(-1,1),  (( results[:, 9] - QIE_true )**2).reshape(-1,1)  ) )## QDE_mse ## QIE_mse
    
    indicators = results_all[:, [0,1,2,3] ] < 0.05
    ...
    col_names=['pvalue_QDE_twoSides',  'pvalue_normal_QDE_twoSides',
               'pvalue_QDE_RightSide', 'pvalue_normal_QDE_RightSide',
               'QDE','Time', 'QDE_indicator_twoSides', 'QDE_norm_indicator_twoSides',
                           'QDE_indicator_RightSide','QDE_norm_indicator_RightSide']
 ...
 ########## results output ######################
    folder_path ='./noInterference/ResidMultiple_NN_'+str(NN)+'_TI_'+str(TI)
